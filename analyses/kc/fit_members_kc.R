...
kc_set <- read_as_workflow_set(file.path("analyses", "kc", "candidate_fits"))

load(file.path("analyses", "kc", "kc_data.RData"))
...
if (!dir.exists(file.path("analyses", "kc", "member_fits"))) {
  dir.create(file.path("analyses", "kc", "member_fits"))
}

for (i in seq_along(kc_member_fits)) {
  obj_nm <- kc_member_fits_names[i]
  obj <- kc_member_fits[[i]]
  file_nm <- file.path("analyses", "kc", "member_fits", paste0(obj_nm, ".RData"))
