...
caret_set <- read_as_workflow_set(file.path("analyses", "caret", "candidate_fits"))

load(file.path("analyses", "caret", "caret_data.RData"))
...
if (!dir.exists(file.path("analyses", "caret", "member_fits"))) {
  dir.create(file.path("analyses", "caret", "member_fits"))
}

for (i in seq_along(caret_member_fits)) {
  obj_nm <- caret_member_fits_names[i]
  obj <- caret_member_fits[[i]]
  file_nm <- file.path("analyses", "caret", "member_fits", paste0(obj_nm, ".RData"))
