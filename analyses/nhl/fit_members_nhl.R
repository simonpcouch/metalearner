...
nhl_set <- read_as_workflow_set(file.path("analyses", "nhl", "candidate_fits"))

load(file.path("analyses", "nhl", "nhl_data.RData"))
...
nhl_model_stack <-
  nhl_data_stack %>%
  blend_predictions(times = 3, meta_learner = meta_learner = null_model() %>% set_engine("parsnip")) %>%
  fit_members()
...
if (!dir.exists(file.path("analyses", "nhl", "member_fits"))) {
  dir.create(file.path("analyses", "nhl", "member_fits"))
}

for (i in seq_along(nhl_member_fits)) {
  obj_nm <- nhl_member_fits_names[i]
  obj <- nhl_member_fits[[i]]
  file_nm <- file.path("analyses", "nhl", "member_fits", paste0(obj_nm, ".RData"))
