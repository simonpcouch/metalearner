...
hpc_set <- read_as_workflow_set(file.path("analyses", "hpc", "candidate_fits"))

load(file.path("analyses", "hpc", "hpc_data.RData"))
...
if (!dir.exists(file.path("analyses", "hpc", "member_fits"))) {
  dir.create(file.path("analyses", "hpc", "member_fits"))
}

for (i in seq_along(hpc_member_fits)) {
  obj_nm <- hpc_member_fits_names[i]
  obj <- hpc_member_fits[[i]]
  file_nm <- file.path("analyses", "hpc", "member_fits", paste0(obj_nm, ".RData"))
