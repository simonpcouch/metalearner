...
rare_set <- read_as_workflow_set(file.path("analyses", "rare", "candidate_fits"))

load(file.path("analyses", "rare", "rare_data.RData"))
...
if (!dir.exists(file.path("analyses", "rare", "member_fits"))) {
  dir.create(file.path("analyses", "rare", "member_fits"))
}

for (i in seq_along(rare_member_fits)) {
  obj_nm <- rare_member_fits_names[i]
  obj <- rare_member_fits[[i]]
  file_nm <- file.path("analyses", "rare", "member_fits", paste0(obj_nm, ".RData"))
