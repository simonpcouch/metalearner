...
barley_set <- read_as_workflow_set(file.path("analyses", "barley", "candidate_fits"))

load(file.path("analyses", "barley", "barley_data.RData"))
...
if (!dir.exists(file.path("analyses", "barley", "member_fits"))) {
  dir.create(file.path("analyses", "barley", "member_fits"))
}

for (i in seq_along(barley_member_fits)) {
  obj_nm <- barley_member_fits_names[i]
  obj <- barley_member_fits[[i]]
  file_nm <- file.path("analyses", "barley", "member_fits", paste0(obj_nm, ".RData"))
