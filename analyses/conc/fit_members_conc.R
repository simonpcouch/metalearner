...
conc_set <- read_as_workflow_set(file.path("analyses", "conc", "candidate_fits"))

load(file.path("analyses", "conc", "conc_data.RData"))
...
if (!dir.exists(file.path("analyses", "conc", "member_fits"))) {
  dir.create(file.path("analyses", "conc", "member_fits"))
}

for (i in seq_along(conc_member_fits)) {
  obj_nm <- conc_member_fits_names[i]
  obj <- conc_member_fits[[i]]
  file_nm <- file.path("analyses", "conc", "member_fits", paste0(obj_nm, ".RData"))
