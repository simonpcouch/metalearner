...
wind_set <- read_as_workflow_set(file.path("analyses", "wind", "candidate_fits"))

load(file.path("analyses", "wind", "wind_data.RData"))
...
if (!dir.exists(file.path("analyses", "wind", "member_fits"))) {
  dir.create(file.path("analyses", "wind", "member_fits"))
}

for (i in seq_along(wind_member_fits)) {
  obj_nm <- wind_member_fits_names[i]
  obj <- wind_member_fits[[i]]
  file_nm <- file.path("analyses", "wind", "member_fits", paste0(obj_nm, ".RData"))
